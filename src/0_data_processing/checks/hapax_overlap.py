...
Usage:
    python src/0_data_processing/hapax_overlap.py            # all languages
    python src/0_data_processing/hapax_overlap.py ces eng    # selected languages
"""
...
def _compute_pairs(lang: str, files: list[Path]) -> list[PairResult]:
    ...
    for path in files:
        print(f"  Reading hapaxes from {path.name} …")
    ...
    # Keep only top MAX_PAIRS_PER_LANG by highest max(share_a, share_b)
...
def main(argv: Optional[list[str]] = None) -> None:
    ...
    # Only process languages with at least 2 files
    ...
    for lang in sorted(groups):
        ...
        print(f"\n[{lang}] {len(files)} file(s), {n_pairs} pair(s):")
        results = _compute_pairs(lang, files)
        all_results.extend(results)

        for r in sorted(results, key=lambda r: max(r.share_a_pct, r.share_b_pct),
                         reverse=True):
            print(f"    {r.file_a}  ↔  {r.file_b}")
            print(f"      hapaxes: {r.hapaxes_a:,} / {r.hapaxes_b:,}  "
                  f"overlap: {r.overlap:,}  "
                  f"share: {r.share_a_pct:.2f}% / {r.share_b_pct:.2f}%")
        print(f"{'-' * 60}")
